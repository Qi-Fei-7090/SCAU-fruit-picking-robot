	...
	while (ros::ok())
	{
		...
		if(scan_vec_ready == 1)
		{
			...
			for(int i =0 ;i < scan_vec.size();i++)
			{
				if (!inverted)
				{
					iter_x[0] = (sin(DEG2RAD(scan_vec[i].angle + angle_offset) / 100) * (float)scan_vec[i].dist) / 1000;//x
					iter_x[1] = (cos(DEG2RAD(scan_vec[i].angle + angle_offset) / 100) * (float)scan_vec[i].dist) / 1000;//y
				}
				else
				{
