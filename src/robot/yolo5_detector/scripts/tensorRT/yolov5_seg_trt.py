...
from com.datastruct import bbox2d,result
...
class YoLov5segTRT(object):
    ...
    def infer(self, raw_image,img_originHW=(480,640)):
        ...
        batch_input_image = np.ascontiguousarray(input_image)
        ...
        for i in range(self.batch_size):
            ...
            print(result_masks.shape,result_masks.dtype)
            ...
            for j in range(len(result_boxes)):
                ret = result()
                ret.bbox.xyxy = result_boxes[j]
                ret.id = result_classid[j]
                ret.prob = result_scores[j]
                ret.mask =result_masks[j]
    ...
    def post_process(self, output_boxes, origin_h, origin_w):
        ...
        print(len(pred),len(boxes),origin_h, origin_w,CONF_THRESH,IOU_THRESHOLD)
    ...
    def process_mask(self, output_proto_mask, result_proto_coef, result_boxes, ih, iw):
        ...
        c, mh, mw = self.seg_c, self.seg_h, self.seg_w#result_proto_masks.shape
        t = np.einsum('ij,jk->ik',result_proto_coef,output_proto_mask.reshape(c, -1))  
        #t = result_proto_coef @ output_proto_mask.reshape(c, -1)
        #t = (result_proto_coef @ )
        masks = self.sigmoid(t).reshape(-1, mh, mw)
        mask_result = []
